...
public:
    ...
    ComplexMat_(const T mat[][2], const int mat_cols,const int mat_rows) : cols(mat_cols), rows(mat_rows), n_channels(1)
    {
	p_data.push_back(convert(mat));
    }
    ...
    void set_channel(int idx, const T mat[][2])
    {
        assert(idx >= 0 && idx < n_channels);
        p_data[idx] = convert(mat);
    }
	...
	ComplexMat_<T> div(const ComplexMat_<T> & rhs) const
	{
		return matn_mat1_operator([](std::complex<T> & c_lhs, const std::complex<T> & c_rhs) { c_lhs /= c_rhs; }, rhs);
	}
...
private:
    ...
    std::vector<std::complex<T>> convert(const T mat[][2])
    {
        std::vector<std::complex<T>> result;
        result.reserve(cols*rows);
        for (int y = 0; y < rows*cols; y++) {
            result.push_back(std::complex<T>(mat[y][0], mat[y][1]));
        }
        return result;
    }
